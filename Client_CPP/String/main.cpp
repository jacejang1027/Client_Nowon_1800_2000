...
int main() {
	...
	//char lastName[DEFAULT_SIZE]; // 한 문자를 1byte로 사용함.
	//wchar_t ct = L'아'; // 한 문자를 2byte로 사용함.
	...
	cout << "성을 입력하세요";
	//cin >> firstName;
	//cin.get(firstName, DEFAULT_SIZE)
	//cin.get(firstName, DEFAULT_SIZE).ignore(DEFAULT_SIZE, '\n');
	getline(cin, firstName);

	cout << "이름을 입력하세요";
	//cin >> lastName;
	//cin.get(lastName, DEFAULT_SIZE)
	...
	// compare()
	// 두 문자열을 비교하는 메소드
	// 같은경우 0, 인자보다 사전순으로 앞에있을경우 음수, 뒤에있을경우 양수
	cout << (str1.compare(str2) == 0) << endl;
	cout << (str1.compare(str2) < 0) << endl;
	cout << (str1.compare(str2) > 0) << endl;

	// replace()
	// 인자로 넣어준 문자열을 찾아서 다른 문자열로 대체하는 메소드
