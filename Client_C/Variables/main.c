...
int main(void) {
	...
	//unsigned : 다른 자료형에도 앞에 붙어서 부호없는 상태로 사용 가능하게 한다.
	unsigned int ua; // 4byte, 부호가 없는 정수 자료형, unsigned 키워드는 
	short s; // 2byte, 부호가 있는 정수 자료형
	long l; // 4byte, 부호가 있는 정수 자료형
	long long ll; // 8byte 이상, 부호가 있는 정수 자료형
	char c; // 1byte(대부분, c언어에서는 4byte), 문자 자료형
	...
	a = 4;
	ua = 0;
	c = 1232; // -128 ~ 127 까지 범위, 1232를 대입하면 오버플로우 발생
	s = 1232;
	printf("%d\n", a);
	printf("%d\n", ua);
	printf("%d\n", c);
	...
	f = 1.23456789123456789123456789123456789;
	d = 1.23456789123456789123456789123456789;
	ld = 1.23456789123456789123456789123456789;
	...
	//stromp(문자열값1, 문자열값2) : 문자열을 아스키코드로 변환했을때
	// 문자열 값1이 더크면 양수 반환, 같으면 0 반환, 작으면 -1 반환
	printf("%d\n", strcmp(str3, str4));
	...
	// LValue 와 RValue
	// 대입연산자 기준으로 왼쪽에 있는 값을 LValue 오른쪽에 있는 값을 RValue 라고함
	//---------------------------------------------------------------------
