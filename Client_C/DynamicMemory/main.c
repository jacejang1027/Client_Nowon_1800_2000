...
// 크기를 미리 정해줄 수 없고, 조건에 따라 맞는 크기를 확보해야하는 경우가 있으니까
...
//
// 쓰는 이유 :
// 이미 할당했지만 할당한 공간의 크기를 변경하고 싶을 때
// 기존 쓰던 주소를 유지하고싶을 때
...
int main(int argc, char **argv) {
	...
	// 함수 호출스택이 해제될때, 해당 변수의 주소의 메모리는 같이 해제되기 떄문에 
	// free 함수를 통해서 해당 메모리영역을 해제해줄 필요 없지만,
	// 동적 할당된 힙 영역의 메모리는 함수가 스택에서 해제되어도
	// 힙 영역의 해당 메모리영역이 해제되지 않기 때문에 힙영역에 할당한 메모리의 주소를
