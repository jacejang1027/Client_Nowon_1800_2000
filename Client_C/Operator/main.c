...
int main(void) {
	...
	// / 연산자
	// 정수의 나누기 연산은 정수 몫만 반환한다. (소수 이하 버림)
	intResult = 3 / 4;
	doubleResult = 3.0 / 4.0;
	printf("%d\n", intResult);
	printf("%lf\n", doubleResult);
	...
	// | ( 비트 or 연산자. 피연산자 2개. )
	// 비교할 두 비트가 모두 1이면 해당자리 비트 1, 나머지 경우 0
	printf("%d | %d = %d\n", 5, 10, 5 | 10);
	...
	// << ( 비트 쉬프트 래프트 )
	// 모든 비트를 왼쪽으로 밈. 초과하는 비트(오버플로우 비트)는 사라짐
	printf("shift %d to the left by %d = %d\n", 5, 3, 5 << 3);
	
	// >> ( 비트 쉬프트 라이트 )
	// 모든 비트를 오른쪽으로 밈. 초과하는 비트(오버플로우 비트)는 사라짐
	...
	printf("%d, %d, %d", size_int, size_size_int, size_intConstant);
	...
	char num_char = 4;
	num_short + num_char;
	// short + char 를 했지만 연산을 하기 위해 레지스터에 데이터가 전달될때 
	...
	// 이런 경우 따로 프로그래머가 int 형으로 바꿔주지 않아도 되어서 암시적 형변환 이라고 함.
