...
int main() {
	...
	// 하드디스크로부터 파일을 읽어들여서 FILE Steram 의 버퍼에 등록하는 함수
	// 
	// 파일 개방 모드
	...
	fputc('c', fp2);
	fclose(fp2); 
	// 쓰기모드에서 버퍼에 들어있는 파일의 내용 수정이 끝나자마자 fclose 를 호출해주는것이 안전하다.
	// 버퍼에서 해체되기 전까지는 하드디스크에 수정한 내용이 반영이 안되기 때문.
	...
	if (fp4 == NULL)
	{
		printf("failed to create chulsu.txt");
		exit(0);
	}

	fp5 = fpopen("youngheui.txt", "w");
	if (fp5 == NULL)
	{
		printf("failed to create youngheui.txt");
		exit(1);
	}
